...
| P5-1 | 5 | Run side-by-side validation | Copilot | IN_PROGRESS | 2026-03-20T06:02:11Z |  | Runtime findings captured: (1) UI produced repeated 502 due to stale nginx upstream target after API container replacement; immediate mitigation was UI restart, long-term mitigation is startup ordering and operational restart guidance. (2) Ollama runner startup timeouts observed under load contention (`timed out waiting for llama runner to start: context canceled`); mitigation applied with `OLLAMA_NUM_PARALLEL=1`, `OLLAMA_MAX_LOADED_MODELS=1`, `OLLAMA_KEEP_ALIVE=30m` plus `api`/`ingestor` dependency on healthy `ollama`. Files: docker-compose.yml, .env.example, README.md. |
